...
resource "aws_subnet" "project4_subnet" {
  ...
  tags = {
    Name = "project4_subnet-${count.index}"
  }
}
...
resource "aws_route_table" "project4_route_table" {
  ...
  tags = {
    Name = "project4_route_table"
  }
}

resource "aws_route_table_association" "a" {
  count          = 2
  subnet_id      = aws_subnet.project4_subnet[count.index].id
  route_table_id = aws_route_table.project4_route_table.id
}

resource "aws_security_group" "project4_cluster_sg" {
  vpc_id = aws_vpc.project4_vpc.id
...
resource "aws_eks_cluster" "project4" {
  ...
  vpc_config {
    ...
    security_group_ids = [aws_security_group.project4_cluster_sg.id]
  }
}

resource "aws_eks_node_group" "project4" {
  cluster_name    = aws_eks_cluster.project4.name
  node_group_name = "project4-node-group-v3"
...
resource "aws_iam_role" "project4_node_group_role" {
  name = "project4-node-group-role-v3"
